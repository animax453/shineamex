...
;(function($) {
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                ...
                if (row.is('TR')) {
                    ...
                    row.children(':last').append('<em style="width:135px;"><a class="' + options.deleteCssClass +'" href="javascript:void(0)">' + options.deleteText + '</a></em>');
                } else if (row.is('UL') || row.is('OL')) {
                    // If they're laid out as an ordered/unordered list,
                    // insert an <li> after the last list item:
                    row.prepend('<em style="width:135px;"><a class="' + options.deleteCssClass + '" href="javascript:void(0)">' + options.deleteText +'</a></em>');
                } else {
                ...
                row.find('a.' + options.deleteCssClass).click(function() {
                    var row = $(this).parents('.' + options.formCssClass), del = row.find('input:hidden[id $= "-DELETE"]');
                    if (del.length) {
        ...
        $$.each(function(i) {
            var row = $(this), del = row.find('input:checkbox[id $= "-DELETE"]');
            if (del.length) {
        ...
        if ($$.length) {
            ...
            if ($$.attr('tagName') == 'TR') {
                ...
                $$.filter(':last').after('<div class="addanother"><a class="' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</a></div>');
